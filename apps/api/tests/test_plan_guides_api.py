...
def test_guide_program_catalog_contract() -> None:
    ...
    full_body = next((item for item in payload if item["id"] == "pure_bodybuilding_phase_1_full_body"), None)
...
def test_program_guide_day_and_exercise_endpoints() -> None:
    ...
    program = client.get("/plan/guides/programs/pure_bodybuilding_phase_1_full_body")
    assert program.status_code == 200
    program_payload = program.json()
    assert program_payload["id"] == "pure_bodybuilding_phase_1_full_body"
    ...
    day = client.get("/plan/guides/programs/pure_bodybuilding_phase_1_full_body/days/1")
    assert day.status_code == 200
    day_payload = day.json()
    assert day_payload["program_id"] == "pure_bodybuilding_phase_1_full_body"
    assert day_payload["day_index"] == 1
    assert len(day_payload["exercises"]) >= 1

    first_exercise_id = day_payload["exercises"][0]["id"]
    exercise = client.get(f"/plan/guides/programs/pure_bodybuilding_phase_1_full_body/exercise/{first_exercise_id}")
    assert exercise.status_code == 200
    exercise_payload = exercise.json()
    assert exercise_payload["program_id"] == "pure_bodybuilding_phase_1_full_body"
...
def test_program_guide_endpoint_404s_for_unknown_resources() -> None:
    ...
    missing_day = client.get("/plan/guides/programs/pure_bodybuilding_phase_1_full_body/days/999")
    assert missing_day.status_code == 404

    missing_exercise = client.get("/plan/guides/programs/pure_bodybuilding_phase_1_full_body/exercise/unknown_exercise")
    assert missing_exercise.status_code == 404


def test_program_guide_endpoints_keep_legacy_alias_urls_compatible() -> None:
    _reset_db()
    client = TestClient(app)
    ...
    day = client.get("/plan/guides/programs/adaptive_full_body_gold_v0_1/days/1")
    assert day.status_code == 200
    day_payload = day.json()
    assert day_payload["program_id"] == "pure_bodybuilding_phase_1_full_body"
